...
class AFNO1D(nn.Module):
    ...
    def __init__(
        ...
        freq_count = 1085 // 2 + 1
        self.mask_token = nn.Parameter(torch.zeros(1, freq_count, 1, 1, 1))
        # self.filter_mlp = nn.Linear(hidden_size,hidden_size,bias=True)
        # self.w = nn.Parameter(self.scale * torch.rand(2,hidden_size,hidden_size))
        # self.b = nn.Parameter(self.scale * torch.rand(2,hidden_size))
    ...
    def forward(self, x):
        ...
        # imaginary_part = torch.imag(x)

        # real_part = F.relu(torch.einsum('bnc,cd->bnd', real_part, self.w[0]) + self.b[0])
        # imaginary_part = F.relu(torch.einsum('bnc,cd->bnd', imaginary_part, self.w[1]) + self.b[1])
